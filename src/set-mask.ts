import type { SimpleMaskMoneyConfiguration, HTMLInputElementMasked } from 'src/types';
 ...
 */
function setMask(input: HTMLInputElementMasked | HTMLInputElement | string | null, configuration?: Partial<SimpleMaskMoneyConfiguration>) {
  ...
  const {
    allowNegative, fractionDigits, decimalSeparator, thousandsSeparator, fixed, prefix, suffix, cursor
  ...
  if (!(element instanceof HTMLInputElement)) throw new Error('the element must be an input');
  ...
  const setCaretPosition = () => {
    const position = cursor === 'end'
      ? [element.value.length - suffix.length, element.value.length - suffix.length] as const
      : [0, 0] as const;
    element.focus();
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    // Allow only number
    if (!allowedKeys.includes(e.key) && (e.key !== '-' || !allowNegative)) return;
    ...
    if (e.key === 'Backspace' && (suffix.length > 0 && start > element.value.length - suffix.length)) return;
    ...
    const remove = length === 0
      ? [start - 1, 1] as const
      : [start, length] as const;

    // Define a range to add
    const add = [start, 0, e.key] as const;

    // Add or remove characters
    characteres.splice(...((e.key === 'Backspace' ? remove : add) as [number, number]));

    characteres = removeSuffix(removePrefix(characteres));

    while (characteres[0] === '0' || characteres[0] === decimalSeparator || characteres[0] === thousandsSeparator)
      characteres.shift();

    if (characteres.length <= fractionDigits)
      return triggerInputChanges(addPrefixAndSuffix(`${completer}${decimalSeparator}${fillDecimals(characteres.join(''))}`));
...
export function removeMask(input?: HTMLInputElementMasked | HTMLInputElement | string | null): () => void {
